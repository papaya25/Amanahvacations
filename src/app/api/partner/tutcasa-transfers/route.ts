  ...
  const { error } = await supabase.from("tutcasa_transfers").upsert(
    {
      ...
      passengers: Number(body.passengers) || (Number(body.adults) || 0) + (Number(body.kids) || 0) || null,
      adults: Number(body.adults) || null,
      kids: Number(body.kids) || null,
      kids_ages: str(body.kidsAges),
