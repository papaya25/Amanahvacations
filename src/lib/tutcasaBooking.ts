...
export async function sendTransferReminders(): Promise<void> {
  try {
    ...
    const { data } = await supabase
      ...
      .select("transfer_id, ref, full_name, travel_date, flight_number, passengers, baby_seat, guest_phone, home, note, kind, provider, price, adults, kids, kids_ages")
      .eq("status", "confirmed")
    ...
    const lines = data.map((t) => {
      ...
      return (
        ...
        `  Flight: ${t.flight_number ?? "—"} · ${t.adults != null ? `${t.adults} adults${t.kids ? ` + ${t.kids} kids (ages: ${t.kids_ages ?? "?"})` : ""}` : `${t.passengers ?? "?"} pax`}${t.baby_seat ? " · BABY SEAT" : ""}\n` +
